...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=(3,3), dilation=1, stride=1, padding=1, bias=True, transpose=False, spectral=True, relu=True, norm=True):
        super().__init__()
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.initial = nn.Sequential(
            ConvBlock(in_channels=512, out_channels=512, kernel_size=4, padding=0, bias=False, transpose=True),
        ...
        self.layers = nn.ModuleList([self.initial, self.g1, self.g2, self.g3, self.g4, self.g5, self.g6])
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.initial = nn.Sequential(
            ConvBlock(in_channels=3, out_channels=64, kernel_size=(1, 1), padding=0, spectral=False, norm=False),
            MiniBatchStd(),
            ConvBlock(in_channels=65, out_channels=64, spectral=False, norm=False),
            ConvBlock(in_channels=64, out_channels=128, spectral=False, norm=False),
            nn.AvgPool2d(2)
        )
        ...
        self.d1 = nn.Sequential(
            MiniBatchStd(),
            ConvBlock(in_channels=132, out_channels=128, spectral=False, norm=False),
            ConvBlock(in_channels=128, out_channels=256, spectral=False, norm=False),
            nn.AvgPool2d(2)
        )
        ...
        self.d2 = nn.Sequential(
            MiniBatchStd(),
            ConvBlock(in_channels=260, out_channels=256, spectral=False, norm=False),
            ConvBlock(in_channels=256, out_channels=512, spectral=False, norm=False),
            nn.AvgPool2d(2)
        )
        ...
        self.d3 = nn.Sequential(
            MiniBatchStd(),
            ConvBlock(in_channels=516, out_channels=512, spectral=False, norm=False),
            ConvBlock(in_channels=512, out_channels=512, spectral=False, norm=False),
            nn.AvgPool2d(2)
        )
        ...
        self.d6 = nn.Sequential(
            MiniBatchStd(),
            ConvBlock(in_channels=516, out_channels=512, spectral=False, norm=False),
            ConvBlock(in_channels=512, out_channels=512, kernel_size=(4, 4), padding=0, spectral=False, norm=False),
            nn.Flatten(),
            nn.Linear(512, 1),
            nn.Sigmoid()
        )
        self.layers = nn.ModuleList([self.initial, self.d1, self.d2, self.d3, self.d4, self.d5, self.d6])
...
if __name__ == '__main__':
    ...
    for layer in layers:
        ...
        layer = layer * 0.5 + 0.5
        print(torch.min(layer).item(), torch.max(layer).item())
